...
some of those calls sit inside compiler-generated closures where a search for
callers of the method token finds nothing. `BuildingUpdate` really is never called;
`KeyEmulationUpdate` looks identical to that search and is very much alive. Check
the handler for a forwarder before concluding either way.
...
| File | What is in it |
| --- | --- |
| [01-loader-and-blacklist.md](notes/01-loader-and-blacklist.md) | mod layout, the namespace blacklist and its carve-outs, the ancient C# compiler, module attributes |
...
| [04-ui-factory.md](notes/04-ui-factory.md) | why Besiege's own UI cannot be borrowed, what UI Factory ships, and how to depend on it softly |
...
| [06-reading-the-game.md](notes/06-reading-the-game.md) | Cecil, probe compiles, unpacking asset bundles, and where the logs are |
...
| [12-machines-and-saves.md](notes/12-machines-and-saves.md) | the autosave folder, reading a `.bsg` without file access, `MachineInfo`/`BlockInfo`, blocks that are several blocks |
...
- **Orchestra** — nine instrument blocks; audio on the mixer thread, a docked UI
  Factory panel, generated machines. Most of the automation and docking notes.
- **Special Effects** — particle, light, glass and text blocks. The key-emulation
  split came from there.
